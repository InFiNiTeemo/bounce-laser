...
export function updateEnemyBullets(dt){for(let i=game.enemyBullets.length-1;i>=0;i--){const eb=game.enemyBullets[i];eb.x+=eb.vx;eb.y+=eb.vy;eb.life--;if(eb.x<-10||eb.x>W+10||eb.y<-10||eb.y>H+10||eb.life<=0){game.enemyBullets.splice(i,1);continue;}checkBulletPortalCollision(eb);if(game.shieldActive&&Math.hypot(eb.x-player.x,eb.y-player.y)<SHIELD_RADIUS){spawnParticles(eb.x,eb.y,COLORS.shield,6);game.shieldEnergy=Math.max(0,game.shieldEnergy-5);game.enemyBullets.splice(i,1);game.score+=10;continue;}if(player.invincibleTimer<=0&&Math.hypot(eb.x-player.x,eb.y-player.y)<player.size+3){game.enemyBullets.splice(i,1);if(damagePlayer('\u88AB\u654C\u4EBA\u7684\u5B50\u5F39\u51FB\u6740\u4E86!'))return;continue;}for(let j=game.bullets.length-1;j>=0;j--){const pb=game.bullets[j];if(Math.hypot(eb.x-pb.x,eb.y-pb.y)<8){spawnParticles(eb.x,eb.y,COLORS.warning,6);game.enemyBullets.splice(i,1);game.bullets.splice(j,1);game.score+=25;break;}}}}
