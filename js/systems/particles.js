...
export function drawParticles(){game.particles.forEach(p=>{ctx.globalAlpha=p.life/p.maxLife;drawPixelRect(p.x-p.size/2,p.y-p.size/2,p.size,p.size,p.color);});ctx.globalAlpha=1;}
